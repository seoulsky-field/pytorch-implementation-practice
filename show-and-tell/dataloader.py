...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, path, json, vocab=None, transform=None):
        ...
        self.ids = list(self.coco.anns.keys()) # unique identifiers for annotations
        self.vocab = vocab
...
def collate_fn(data):
    ...
    :param data: list; (image, caption) tuples
            ...
            - cpation: tensor;  1 x length_caption
    
    Return: mini-batch
    :return images: tensor;     batch_size x 3 x 256 x 256
    :return padded_captions: tensor;    batch_size x length
    :return caption_lengts: list;   lengths of actual captions (without padding)"""
...
def get_coco_data_loader(path, json, vocab, transform=None, batch_size=32, shuffle=True, num_workers=2):
...
def get_basic_loader(dir_path, transform, batch_size=32, shuffle=True, num_workers=2):
    ...
    # lengths: list indication valid length for each caption. length is (batch_size).
    data_loader = data.DataLoader(dataset=datas, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
